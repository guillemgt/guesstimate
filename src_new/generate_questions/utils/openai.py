...
class OpenAIBatchHandler:
    ...
    def _get_file_path(self, batch_call_id: str, index: int, suffix: str):
        if self.log_dir is None:
            ...
            return os.path.join(self.log_dir, batch_call_id, f"{index}-{suffix}.{extension}")
        
    def _write_metadata(self, metadata: BatchInfo, batch_call_id: str, index: int):
        if self.log_dir is None:
            return
        with open(self._get_file_path(batch_call_id=batch_call_id, index=index, suffix="metadata"), "w", encoding="utf-8") as f:
    ...
    def download_batch(self, metadata: BatchInfo):
        ...
        if self.log_dir is not None:
            with open(self._get_file_path(batch_call_id=metadata.batch_call_id, index=metadata.index, suffix="output"), "w", encoding="utf-8") as f:
    ...
    def _upload_and_download(
        ...
        if self.log_dir is not None and os.path.exists(self._get_file_path(batch_call_id, index, "metadata")):
            with open(self._get_file_path(batch_call_id, index, "metadata"), "r", encoding="utf-8") as f:
                metadata = json.load(f)
                if metadata["hash"] == whole_content_hash or True:
                    file_handler.close()
                    if os.path.exists(self._get_file_path(batch_call_id, index, "output")):
                        decoded_content = open(self._get_file_path(batch_call_id, index, "output"), "r", encoding="utf-8").read()
                        return [json.loads(line) for line in decoded_content.split("\n") if line]
    ...
    def upload_batches(
        ...
        for request_index in range(len(batch_lines)):
            ...
            if (max_tokens_per_batch is not None and tokens_in_batch + tokens_in_request > max_tokens_per_batch) or (max_requests_per_batch is not None and requests_in_batch >= max_requests_per_batch):
                outputs_raw.extend(
